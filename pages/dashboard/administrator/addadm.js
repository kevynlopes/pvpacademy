...
export default function ScreenAddAdm() {
    ...
    const handleSave = () => {
        ...
        if(!cpf || !email || !pass) return;
        
        axios.post('https://api.pvpacademy.com.br:3333/create/user', {
            token: 'Batata',
            cpf: cpf,
            email: email,
            pass: pass,
            plans: ['basico', 'avancado', 'tryhard'],
            adm: true
        }).then((res) => {
    ...
    return (
        <div className={styles.container}>
            ...
            <main>
                ...
                <div className={styles.session}>
                    <div className={styles.painel}>
                        ...
                        <a href="/dashboard/administrator/addadm" className={`${styles.adm}  ${styles.selected}`}>Adicionar administrador</a>
                        <a href="/dashboard/administrator/remadm">Remover administrador</a>

                    </div>
                    <div className={styles.conteudo}>

                        <h1 className={styles.top}>Adicionar administrador</h1>

                        <h2>CPF</h2>
                        <input onChange={(c) => setCPF(c.target.value)} placeholder="Insira o cpf..." />

                        <h2>E-mail</h2>
                        <input onChange={(c) => setEmail(c.target.value)} placeholder="Insira o e-mail..." />

                        <h2>Senha</h2>
                        <input onChange={(c) => setPass(c.target.value)} placeholder="Insira a senha..." />
            ...
            {sucess ? <Sucess>Sucesso! Administrador adicionado em nossa plataforma.</Sucess> : null}
        </div>
    )
}